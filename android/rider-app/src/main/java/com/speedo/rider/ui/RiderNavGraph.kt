...
fun RiderMainScaffold(
    ...
    val onRootBackPress: () -> Unit = {
        ...
        if (now - lastBackPressTime < 2000L) {
            ...
            android.widget.Toast.makeText(context, "Press back again to exit Speedo", android.widget.Toast.LENGTH_SHORT).show()
        }
    }
    ...
    BackHandler(enabled = true) {
        if (showExitDialog) {
        ...
        } else if (currentRoute != null && currentRoute != RiderScreen.Home.route) {
            // Single press: navigate back to Home screen!
            ...
            // On home screen: press twice within 2 seconds to confirm exit
            onRootBackPress()
        }
    }
